...
def get_omniweaving_pre_process_func(od_config: OmniDiffusionConfig):
    ...
    def pre_process_func(req: OmniDiffusionRequest) -> OmniDiffusionRequest:
        ...
        if height is None or width is None:
            ...
            req.sampling_params.height = height or (
                (target_h + divisor - 1) // divisor * divisor
            )
            req.sampling_params.width = width or (
                (target_w + divisor - 1) // divisor * divisor
            )
        return req
...
class OmniWeavingPipeline(
    ...
    def __init__(self, *, od_config: OmniDiffusionConfig, prefix: str = ""):
        ...
        safetensors_files = glob.glob(
            os.path.join(ckpt_dir, "**", "*.safetensors"), recursive=True
        )
        ...
        try:
            ...
            self.feature_extractor = SiglipImageProcessor.from_pretrained(
                siglip_path, cache_dir=hf_cache_dir
            )
        ...
        self.vae_scale_factor_temporal = (
            ...
            if hasattr(self.vae, "temporal_compression_ratio")
            else 4
        )
        ...
        self.num_channels_latents = (
            self.vae.config.latent_channels if hasattr(self.vae, "config") else 32
        )
    ...
    def _get_byte5_prompt_embeds(
        ...
        for p in prompt:
            ...
            if glyph_text is None or not self.has_byt5:
                ...
                glyph_text_embeds_mask = torch.ones(
        ...
        return torch.cat(prompt_embeds_list, dim=0), torch.cat(
            prompt_embeds_mask_list, dim=0
        )
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance:
            ...
            ) = self._get_byte5_prompt_embeds(neg_p, device, dtype)
    ...
    def _get_image_embeds(
        self, image: PIL.Image.Image, device: torch.device
    ) -> torch.Tensor:
        image_encoder_dtype = next(self.image_encoder.parameters()).dtype
        pixel_values = self.feature_extractor(
            images=image, do_resize=True, return_tensors="pt"
        ).pixel_values.to(device=device, dtype=image_encoder_dtype)
        return torch.clamp(
    ...
    def _get_image_latents(
        self, image: PIL.Image.Image, height: int, width: int, device: torch.device
    ) -> torch.Tensor:
        image_tensor = (
        ...
        return (
            retrieve_latents(self.vae.encode(image_tensor), sample_mode="argmax")
    ...
    def prepare_cond_latents_and_mask(
        ...
        latent_condition = self._get_image_latents(
            image, height, width, device
        ).repeat(batch, 1, frames, 1, 1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        prompt = (
            prompt_data
            if isinstance(prompt_data, str)
            else prompt_data.get("prompt")
        )
        negative_prompt = (
            None
            if isinstance(prompt_data, str)
            else prompt_data.get("negative_prompt")
        )
        multi_modal_data = (
            prompt_data.get("multi_modal_data", {})
        ...
        enc_tuple = self.encode_prompt(
            prompt, image, device, dtype, negative_prompt, do_cfg
        )
        ...
        self.scheduler.set_timesteps(
            sigmas=np.linspace(1.0, 0.0, num_steps + 1)[:-1], device=device
        )
        ...
        with self.progress_bar(total=len(timesteps)) as pbar:
            for i, t in enumerate(timesteps):
                ...
                timestep_r = (
                    timestep_r.expand(latents.shape[0]).to(latents.dtype)
                ...
                latents = self.scheduler_step_maybe_with_cfg(
                    noise_pred, t, latents, do_true_cfg=bool(negative_kwargs)
                )
                pbar.update()
    ...
    def _translate_tencent_to_hf(
        self, raw_tf: dict[str, torch.Tensor]
        ...
        if "final_layer.adaLN_modulation.1.weight" in raw_tf:
            mapped_tf["norm_out.linear.weight"] = raw_tf[
                "final_layer.adaLN_modulation.1.weight"
            ]
            mapped_tf["norm_out.linear.bias"] = raw_tf[
                "final_layer.adaLN_modulation.1.bias"
            ]
        if "txt_in.input_embedder.weight" in raw_tf:
            mapped_tf["context_embedder.proj_in.weight"] = raw_tf[
                "txt_in.input_embedder.weight"
            ]
            mapped_tf["context_embedder.proj_in.bias"] = raw_tf[
        ...
        for layer in range(2):
            ...
            if f"{p}.self_attn_qkv.weight" in raw_tf:
                ...
                mapped_tf[f"{pref}.norm1.weight"] = raw_tf[f"{p}.norm1.weight"]
                mapped_tf[f"{pref}.norm1.bias"] = raw_tf[f"{p}.norm1.bias"]
                mapped_tf[f"{pref}.norm2.weight"] = raw_tf[f"{p}.norm2.weight"]
                mapped_tf[f"{pref}.norm2.bias"] = raw_tf[f"{p}.norm2.bias"]
                mapped_tf[f"{pref}.ff.net.0.proj.weight"] = raw_tf[f"{p}.mlp.fc1.weight"]
                mapped_tf[f"{pref}.ff.net.0.proj.bias"] = raw_tf[f"{p}.mlp.fc1.bias"]
                mapped_tf[f"{pref}.ff.net.2.weight"] = raw_tf[f"{p}.mlp.fc2.weight"]
                mapped_tf[f"{pref}.ff.net.2.bias"] = raw_tf[f"{p}.mlp.fc2.bias"]
                mapped_tf[f"{pref}.norm_out.linear.weight"] = raw_tf[f"{p}.adaLN_modulation.1.weight"]
                mapped_tf[f"{pref}.norm_out.linear.bias"] = raw_tf[f"{p}.adaLN_modulation.1.bias"]
        ...
        for i in range(100):
            ...
            mapped_tf[f"{t_p}.attn.to_q.weight"] = raw_tf[f"{p}.img_attn_q.weight"]
            mapped_tf[f"{t_p}.attn.to_k.weight"] = raw_tf[f"{p}.img_attn_k.weight"]
            mapped_tf[f"{t_p}.attn.to_v.weight"] = raw_tf[f"{p}.img_attn_v.weight"]
            mapped_tf[f"{t_p}.attn.to_q.bias"] = raw_tf[f"{p}.img_attn_q.bias"]
            mapped_tf[f"{t_p}.attn.to_k.bias"] = raw_tf[f"{p}.img_attn_k.bias"]
            mapped_tf[f"{t_p}.attn.to_v.bias"] = raw_tf[f"{p}.img_attn_v.bias"]
            mapped_tf[f"{t_p}.attn.add_q_proj.weight"] = raw_tf[f"{p}.txt_attn_q.weight"]
            mapped_tf[f"{t_p}.attn.add_k_proj.weight"] = raw_tf[f"{p}.txt_attn_k.weight"]
            mapped_tf[f"{t_p}.attn.add_v_proj.weight"] = raw_tf[f"{p}.txt_attn_v.weight"]
            mapped_tf[f"{t_p}.attn.add_q_proj.bias"] = raw_tf[f"{p}.txt_attn_q.bias"]
            mapped_tf[f"{t_p}.attn.add_k_proj.bias"] = raw_tf[f"{p}.txt_attn_k.bias"]
            mapped_tf[f"{t_p}.attn.add_v_proj.bias"] = raw_tf[f"{p}.txt_attn_v.bias"]
            mapped_tf[f"{t_p}.attn.norm_q.weight"] = raw_tf[f"{p}.img_attn_q_norm.weight"]
            mapped_tf[f"{t_p}.attn.norm_k.weight"] = raw_tf[f"{p}.img_attn_k_norm.weight"]
            mapped_tf[f"{t_p}.attn.norm_added_q.weight"] = raw_tf[f"{p}.txt_attn_q_norm.weight"]
            mapped_tf[f"{t_p}.attn.norm_added_k.weight"] = raw_tf[f"{p}.txt_attn_k_norm.weight"]
            mapped_tf[f"{t_p}.attn.to_out.0.weight"] = raw_tf[f"{p}.img_attn_proj.weight"]
            mapped_tf[f"{t_p}.attn.to_out.0.bias"] = raw_tf[f"{p}.img_attn_proj.bias"]
            mapped_tf[f"{t_p}.attn.to_add_out.weight"] = raw_tf[f"{p}.txt_attn_proj.weight"]
            mapped_tf[f"{t_p}.attn.to_add_out.bias"] = raw_tf[f"{p}.txt_attn_proj.bias"]
            mapped_tf[f"{t_p}.ff.net.0.proj.weight"] = raw_tf[f"{p}.img_mlp.fc1.weight"]
            mapped_tf[f"{t_p}.ff.net.0.proj.bias"] = raw_tf[f"{p}.img_mlp.fc1.bias"]
            mapped_tf[f"{t_p}.ff.net.2.weight"] = raw_tf[f"{p}.img_mlp.fc2.weight"]
            mapped_tf[f"{t_p}.ff.net.2.bias"] = raw_tf[f"{p}.img_mlp.fc2.bias"]
            mapped_tf[f"{t_p}.ff_context.net.0.proj.weight"] = raw_tf[f"{p}.txt_mlp.fc1.weight"]
            mapped_tf[f"{t_p}.ff_context.net.0.proj.bias"] = raw_tf[f"{p}.txt_mlp.fc1.bias"]
            mapped_tf[f"{t_p}.ff_context.net.2.weight"] = raw_tf[f"{p}.txt_mlp.fc2.weight"]
            mapped_tf[f"{t_p}.ff_context.net.2.bias"] = raw_tf[f"{p}.txt_mlp.fc2.bias"]
            mapped_tf[f"{t_p}.norm1.linear.weight"] = raw_tf[f"{p}.img_mod.linear.weight"]
            mapped_tf[f"{t_p}.norm1.linear.bias"] = raw_sd[f"{p}.img_mod.linear.bias"]
            mapped_tf[f"{t_p}.norm1_context.linear.weight"] = raw_tf[f"{p}.txt_mod.linear.weight"]
            mapped_tf[f"{t_p}.norm1_context.linear.bias"] = raw_tf[f"{p}.txt_mod.linear.bias"]
        ...
        for i in range(100):
            ...
            mapped_tf[f"{t_p}.attn.to_q.weight"] = raw_tf[f"{p}.attn_q.weight"]
            mapped_tf[f"{t_p}.attn.to_k.weight"] = raw_tf[f"{p}.attn_k.weight"]
            mapped_tf[f"{t_p}.attn.to_v.weight"] = raw_tf[f"{p}.attn_v.weight"]
            mapped_tf[f"{t_p}.attn.to_q.bias"] = raw_tf[f"{p}.attn_q.bias"]
            mapped_tf[f"{t_p}.attn.to_k.bias"] = raw_tf[f"{p}.attn_k.bias"]
            mapped_tf[f"{t_p}.attn.to_v.bias"] = raw_tf[f"{p}.attn_v.bias"]
            mapped_tf[f"{t_p}.attn.to_out.0.weight"] = raw_tf[f"{p}.attn_proj.weight"]
            mapped_tf[f"{t_p}.attn.to_out.0.bias"] = raw_tf[f"{p}.attn_proj.bias"]
            mapped_tf[f"{t_p}.proj_mlp.weight"] = raw_tf[f"{p}.mlp.fc1.weight"]
            mapped_tf[f"{t_p}.proj_mlp.bias"] = raw_tf[f"{p}.mlp.fc1.bias"]
            mapped_tf[f"{t_p}.proj_out.weight"] = raw_tf[f"{p}.mlp.fc2.weight"]
            mapped_tf[f"{t_p}.proj_out.bias"] = raw_tf[f"{p}.mlp.fc2.bias"]
            mapped_tf[f"{t_p}.norm.linear.weight"] = raw_tf[f"{p}.mod.linear.weight"]
            mapped_tf[f"{t_p}.norm.linear.bias"] = raw_tf[f"{p}.mod.linear.bias"]
        return mapped_tf
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        if any(k.startswith("double_blocks.") for k in raw_tf.keys()):
            logger.info(
                "Detecting Tencent original weight format. Routing through native Auto-Translator..."
            )
        ...
        for name, p in self.transformer.named_parameters():
            if f"transformer.{name}" not in loaded_keys and (
                torch.isnan(p).any() or p.std().item() == 0.0
            ):
