...
class OmniWeavingPipeline(
    ...
    def __init__(self, *, od_config: OmniDiffusionConfig, prefix: str = ""):
        ...
        try:
            ...
            self.has_t5_model = True
        except Exception:
            self.tokenizer_2 = None
            self.text_encoder_2 = None
            self.has_t5_model = False
            self.t5_d_model = 1472
    ...
    def _get_byte5_prompt_embeds(
        ...
        for p in prompt:
            ...
            if glyph_text is None or not self.has_t5_model:
                glyph_text_embeds = torch.zeros(
                    (1, self.tokenizer_2_max_length, self.t5_d_model),
                    device=device,
                    dtype=dtype,
                )
    ...
    def _translate_tencent_to_hf(
        ...
        for layer in range(2):
            ...
            if f"{p}.self_attn_qkv.weight" in raw_tf:
                ...
                map_k(
                    f"{p}.adaLN_modulation.1.weight", f"{pref}.norm_out.linear.weight"
                )
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # Load ByT5
        if self.has_t5_model and raw_t5:
