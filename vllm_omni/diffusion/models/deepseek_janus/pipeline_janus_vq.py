...
class JanusVQDecodePipeline(nn.Module, SupportsModuleOffload, DiffusionPipelineProfilerMixin):
...
def _load_param(module: torch.nn.Module, name: str, tensor: torch.Tensor) -> None:
    """Load a parameter supporting dotted paths and integer indices."""
    parts = name.split(".")
    obj = module
    for part in parts[:-1]:
        if part.isdigit():
